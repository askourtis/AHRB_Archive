...
\section{Overview}
To partition a cloud of points, \verb|AdaptiveHierarchicalRegularBinning.jl| maps each
point to a $1$ dimensional space filling curve, Morton or Z-order curve. The hierarchical
tree is defined by the Morton index of each point.

\begin{example}
  For a $2$-dimensional space a target spatial tree with $2$ levels (other than the root).
  Assume we compute a morton index of $(0100)_2$. The 2 most significant bits address the
  top most level of the tree (the one directly under root).

  The most significant bit
  addresses the first dimension and the value $(0)_2$ signifies that the point lies in
  the left-most half of the dimension.

  The second most significant bit addresses the
  second dimension and a value of $(1)_2$ signifies that the point lies in the right-most
  half of the dimension (typicaly the lower-most half since this is $2$-dimensional,
  right-most is a more generic term used for any number of dimensions).

  % TODO Insert gaphs
\end{example}

Sorting in terms of these morton indices group the points togather for efficient access
when the tree is iterated over. One natural way of sorting these indices is the
\verb|Radixsort| where we sort based on the most significant radix and recurse for the
remaining bits in the same fashion. This type of sorting process respects the nature of
the morton index and thus can offer benefits over other sorting algorithms.

One benefit of the \verb|Radixsort| is that it offers partial sorting capabilities. When
the number of items within a bin is not high enough, there is no reason to continue the
spatial partitioning and therefore there is no reason to continue sorting. Stopping
\verb|Radixsort| at a specific level of recursion translates to stoping the tree at that
level, there is a $1-1$ corespondance of the \verb|Radixsort| recursion and the tree.

\verb|Radixsort| can also be trivialy parallelized, after sorting the based on the most
significant radix the sub-arrays left to sort are mutually independant and thus offer
themselves to easy parallelization.

Sorting based on a radix is not trivial. Any sorting algorithm will work just fine but
since small radix means small number of discrete values \verb|Countsort| is used.
\verb|Countsort| offers trades-off spatial complexity for runtime complexity.
The worst case runtime complexity of the algorithm is $O(n)$, which means that
it scales linearly with the size of the array to sort. While the spatial complexity for
the in-place variant is $O(max(r))$ where $r$ is the selected radix and $max(r)$ is the
maximum value allowed by $r$.

\verb|Countsort| can be parallelized as well, though not trivialy. The parallel variant
requires a larger spatial complexity of $O(t\cdot max(r))$ where $t$ is the number of
tasks allocated for the parallelization. The runtime complexity drops by the same factor
$t$, $O(n/t)$. Spatial complexity is a limites the capabilities of the algorithm but even
for large values of $t$ this auxilary space needed is not a bad trade-off.

An other trade-off to note is the selection of radix. The bigger the radix the larger
the load on the \verb|Countsort|. Since the time complexity of \verb|Countsort| is $O(n)$
and even better for the parallel variant $O(n/t)$ we prefer that most of the work is
delegated to \verb|Countsort| instead of the recursion process of \verb|Radixsort|.
Selecting a large radix though, might have an adverse effect. The auxilary space needed
is based on the max value of the radix which exponatialy grows as the radix grows, meaning
that we quickly run out of memory for larger matrices.

Our implementation is radix-agnostic, meaning that the user can select the radix. But the
default approach is a radix of $16$ bits. $16$ bits means that at the worst case of
morton indices of $128$ bits we will need to recurse $8$ times and the auxilary space needed
is a factor of $2^{16} = 64kB$.
